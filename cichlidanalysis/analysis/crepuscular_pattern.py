...
def crespuscular_daily_ave_fish(rootdir, feature, fish_tracks_ds, change_times_m, bin_size_min=30):
    ...
    num_day_bins, bins_per_h, dawn_border_bottom, dusk_border_bottom, dawn_dusk_border_bottom, border_top, _, _, _, _ = \
        peak_borders(bin_size_min, change_times_m)

    peak_prom = 0.15
    if feature == 'speed_mm':
        border_top = border_top * 200
        dawn_dusk_border_bottom = dawn_dusk_border_bottom * 200
        peak_prom = 7
    ...
    for species_name in fish_tracks_ds.species.unique():
        ...
        fig2, ax2 = plt.subplots(len(fish_daily_ave_feature.columns), 1, figsize=(6, 2*len(fish_daily_ave_feature.columns)))
        ax2[-1].set_ylabel(feature)

        for fish, fish_name in enumerate(fish_daily_ave_feature.columns):
            ax2[fish].axvspan(dawn_s * bins_per_h, dawn_e * bins_per_h,
                              color='wheat', alpha=0.5, linewidth=0)
            ax2[fish].axvspan(dusk_s * bins_per_h, dusk_e * bins_per_h,
                              color='wheat', alpha=0.5, linewidth=0)
            ax2[fish].axvspan(0, dawn_s * bins_per_h,
                              color='lightblue', alpha=0.5, linewidth=0)
            ax2[fish].axvspan(dusk_e * bins_per_h, num_day_bins,
                              color='lightblue', alpha=0.5, linewidth=0)

            x = fish_daily_ave_feature.iloc[:, fish]
            peaks, _ = find_peaks(x, distance=bins_per_h*2, prominence=peak_prom, height=(dawn_dusk_border_bottom, border_top))
            ax2[fish].plot(x)
            ax2[fish].plot(peaks, x[peaks], "o", color="r")
            ax2[fish].title.set_text(fish_name)
        plt.savefig(os.path.join(rootdir, "cres_peaks_daily_{}_peak_prom_{}.png".format(species_name, peak_prom)), dpi=350)
    plt.close('all')
    return


def crespuscular_weekly_fish(rootdir, feature, fish_tracks_ds, change_times_m, bin_size_min=30):
...
def crepuscular_peaks_min(rootdir, feature, fish_tracks_ds, change_times_m, bin_size_min=30, peak_prom=7):
    ...
    num_day_bins, bins_per_h, dawn_border_bottom, dusk_border_bottom, _, border_top, day_border, night_border, _, _ = \
        peak_borders(bin_size_min, change_times_m)
...
def crepuscular_peaks_min_daily(rootdir, feature, fish_tracks_ds, change_times_m, bin_size_min=30, peak_prom=7):
    """ Uses borders to find peaks in the twilight periods (2h -/+ of sunup/down). Finds peak position.
    This version allows you to use different binning (not just 30min)

    # Returns: peak location, dawn/dusk

    :param feature:
    :param fish_tracks_ds:
    :param fish_diel_patterns: df with: 'FishID', 'peak', 'twilight', 'species', 'species_six'
    :return:
    """
    ...
    for species_name in fish_tracks_ds.species.unique():
        ...
        for fish, fish_name in enumerate(fish_daily_ave_feature.columns):
            x = fish_daily_ave_feature.iloc[:, fish]
            dawn_peak, dawn_peak_prop = find_peaks(x, distance=bins_per_h * 2, prominence=peak_prom, height=(dawn_border_bottom, border_top))
            dusk_peak, dusk_peak_prop = find_peaks(x, distance=bins_per_h * 2, prominence=peak_prom, height=(dusk_border_bottom, border_top))

            # fish_peaks data: position of peak within 24h
            # fish_peaks_df = pd.concat([fish_peaks_df_dawn, fish_peaks_df_dusk], axis=0)
            # fish_peaks_df_dawn['twilight'] = 'dawn'
            # fish_peaks_df_dusk['twilight'] = 'dusk'
            fish_peaks_df = ''
            ...
            if dusk_peak.size == 0:
                dusk_peak = [np.NaN]
            d = {'FishID': [fish_name, fish_name], 'peak_loc': [dawn_peak[0], dusk_peak[0]], 'twilight': ['dawn', 'dusk'],
                 'species': [species_name, species_name]}
            fish_peaks_df = pd.DataFrame(data=d)
...
def peak_borders(bin_size_min, change_times_m):
    ...
    return num_day_bins, bins_per_h, dawn_border_bottom, dusk_border_bottom, dawn_dusk_border_bottom, border_top, day_border, night_border, border_bottom_week, border_top_week
