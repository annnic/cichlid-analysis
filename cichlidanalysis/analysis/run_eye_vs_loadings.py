...
if __name__ == '__main__':
    ...
    empty_array = np.empty((tps_data.__len__(), 2))
    ...
    for species_n, species in loadings.species:
        ...
        for specimen_n in specimens_for_species.index:
            ...
            if first:
                species_measures = pd.DataFrame({'species': [specimen['id'].split('_')[1]], 'ID': specimen_data['id'],
                                                 'eye_size_h': eye_size_h, 'eye_size_v': eye_size_v, 'standard_len':
                                                     standard_len})
                first = False
            else:
                species_measures_i = pd.DataFrame({'species': [specimen['id'].split('_')[1]], 'ID': specimen_data['id'],
                                                 'eye_size_h': eye_size_h, 'eye_size_v': eye_size_v, 'standard_len':
                                                     standard_len})
