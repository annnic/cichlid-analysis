...
if __name__ == '__main__':
    ...
    mean_corr_per_fish = pd.read_csv(os.path.join(rootdir, 'mean_corr_per_fish.csv'))
    ...
    mean_corrs_intra_individ = pd.pivot_table(mean_corr_per_fish, index='species', values='corr_coef', aggfunc='mean')
    mean_corrs_intra_individ = mean_corrs_intra_individ.rename(columns={"corr_coef": "corr_coef_intra_individ"})
    ...
    data1 = loadings.pc1
    data2 = mean_corrs_intra_individ.corr_coef_intra_individ
    model, r_sq = run_linear_reg(data1, data2)
    plt_lin_reg(rootdir, data1, data2, model, r_sq)
