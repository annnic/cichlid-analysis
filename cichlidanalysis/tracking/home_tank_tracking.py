...
def pixel_tracker(video_path, background_full, rois, threshold=5, display=True, area_size=0, split_range=False):
    """ Function that takes a video path, a background file, rois, threshold and display switch. This then uses
    background subtraction and thresholding to find pixel changes. Saves out a csv file with pixel changes"""
    ...
    for roi in range(0, len(rois) - 1):
        ...
        if split_name is False:
            filename = video_path[0:-4] + "_tracks_{}_Thresh_{}_roi-{}.csv".format(date, threshold,
                                                                                           roi)
        else:
            range_s = str(split_range[0]).zfill(5)
            range_e = str(split_range[1]).zfill(5)
            filename = video_path[0:-4] + "_tracks_{}_Thresh_{}_Range{}-{}_.csv".format(date, threshold, range_s,
                                                                                                range_e)
...
if __name__ == '__main__':
    ...
    if background_update == 'y':
        ...
        if background_update_files == 'a':
            ...
            update_background(percentile, vid_type="*.avi")
    ...
    if track_videos == 'y':
        ...
        if track_roi == 'y':
            ...
            for idx, val in enumerate(video_files):
                ...
                pixel_tracker(os.path.join(vid_dir, val), background_crop, rois, threshold=35, display=False, area_size=100)
